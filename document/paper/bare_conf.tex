...
Ashley Kleinhans\IEEEauthorrefmark{2} and
Andrew Mundy\IEEEauthorrefmark{3},
...
eDVS sensor, as shown in Figure 1. The robot provides two motors, a 
frequency-controllable laser pointer, two LEDs, and housing for power 
(4 AA batteries). The setup comes with a proprietary WLAN module, enabling 
streaming of sensor data and motor command to and from the robot over 
standard WiFi.
...
system shown in Figure \ref{React}. 
...
the rotation rate is the difference between the two. Motor output values of 
typical individual runs are plotted along
...
rules mapping sensory states to actions. In this section, we build upon these
rules in a less explicit manner, allowing us to create more complex behavior
without hand-coding the exact rules.  In complex situations, it may not be 
...
For example, consider the simple case where we want the robot to turn left at 
...
examples}. In this case, we consider the four individual runs shown in Figure 
5 where the robot's rotation tended to remain positive.  The data from runs
where it turned left are removed.  The data from these positive examples can 
...
The result of this training is shown in Figure \ref{Left}. Unlike 
Figure \ref{Cnt} this shows the robot consistently turning to the left about 
...
To demonstrate this, we now optionally place a \textit{mirror} at the intersection.
...
depending on whether the mirror is present or not.  Figure \ref{Right} shows 
