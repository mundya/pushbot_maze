...
with model:
    ...
    def vision_state(t):
        ...
        if c2 > 1.0:
            c2 = 1.0
        return x, y, c, dx, dy, x2, y2, c2
    vision = nengo.Node(vision_state, size_out=8)

    state = nengo.Ensemble(800, 8, radius=2)
    ...
    nengo.Connection(state, actions.input[0],
                     function=lambda x: 1.0 if x[1] < -0.85 and x[4] > 0 else 0,
    ...
    nengo.Connection(state, nengo.Node(print_state, size_in=8), synapse=0.01)
